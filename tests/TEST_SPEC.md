...
- **E2Eテスト (Playwright):** ヘッドレスブラウザを制御し、実際のユーザーインタラクション、ドラッグ＆ドロップ、ウィジェット描画（コンパクトなウェイトスライダーなど）、キーボードショートカット、プロンプト大辞典とのポータル双方向連携、およびタイピング中・削除時の誤閉鎖防止を検証します。
...
- **ローディング制御:** ファイルの読み込みや保存など非同期API連携中の `isLoading` と `loadingText` カウンタ状態制御の連動ロジックを検証（2件）。

### 2.3 E2Eテスト (`Playwright` - 11件)
